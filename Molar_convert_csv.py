...
def load_hormones_from_csv(csv_file):
    """Load hormone data from a CSV file."""
    hormones = {}
    with open(csv_file, 'r') as file:
        reader = csv.reader(file)
        for row in reader:
            if len(row) >= 2:
                hormone_name = row[0].strip()
                molecular_weight = float(row[1].strip())
                hormones[hormone_name] = molecular_weight
    return hormones
...
def calculate_mass(hormone_name, molarity, unit, media_volume_liters, m_hormones):
...
def main():
    try:
        csv_file = input("Enter the path to the CSV file containing hormone data: ")
        m_hormones = load_hormones_from_csv(csv_file)
        ...
        for _ in range(num_hormones):
            ...
            Mass_micrograms = calculate_mass(hormone_name, Mol_concentration, unit, media_volume_liters, m_hormones)
